...
class SqsHandler:
    def __init__(self, location):
        ...
        self.sqs_resource: ServiceResource = AWSBackend().get_resource('sqs', region=location)
        self.logger = logging.getLogger(SqsHandler.__class__.__name__)

    def create_queue(self, name, attributes={}):
        try:
            queue: Queue = self.sqs_resource.create_queue(
                QueueName=name,
    ...
    def get_queue(self, name, raiseit: bool = True):
        try:
            queue: Queue = self.sqs_resource.get_queue_by_name(QueueName=name)
    ...
    def get_queue_by_url(self, url):
        return self.sqs_resource.Queue(url=url)
    ...
    def get_queues(self, prefix=None):
        if prefix:
            queue_iter = self.sqs_resource.queues.filter(QueueNamePrefix=prefix)
        else:
            queue_iter = self.sqs_resource.queues.all()
    ...
    def send_message_by_url(self,queue_url,message_body, message_attributes={},config = None):
        sqs_client:Client = AWSBackend().get_client('sqs',region=self._location,config= config)
        try:
    ...
    def receive_messages(self, queue, max_number, wait_time=None, attribute_names=None):
        try:
    ...
    @staticmethod
    def apply_queue_policy(queue, policy):
