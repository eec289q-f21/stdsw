...
class SendMsg(QueueCommand):
    def execute(self):
        ...
        if self._unsigned:
            ...
            return sqs.send_message_by_url(self._qpath.path,self._msg.flatten(),message_attrs,config=Config(signature_version=UNSIGNED))
        else:
            queue = sqs.get_queue_by_url(self._qpath.path)
            return sqs.send_message(queue, self._msg.flatten(), message_attrs)

    def __init__(self, serverpath: Path, queuepath: resources.URL, msg: AWSMsg, anonymous:bool = False):
        super().__init__(serverpath, queuepath)
        self._msg = msg
        self._unsigned = anonymous
